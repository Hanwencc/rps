...
- 控制器/客户端连接模型：
  ...
  - 使用 `client_id + psk` 识别 agent，并通过 Noise + PSK 完成 bridge 认证和加密。
...
MVP 中 `encrypt` 和 `compress` 只保留配置字段，先不启用业务层二次加密和压缩。agent-controller bridge 默认使用 Noise + PSK 加密。
...
```text
Agent -> Controller: NoisePrelude {
  magic: "RPS1",
  client_id,
  version
}

Noise: NNpsk0_25519_ChaChaPoly_BLAKE2s with DB psk

Encrypted Agent -> Controller: Hello {
  magic: "RPS1",
  role: "control" | "data",
  client_id,
  version
...
Encrypted Controller -> Agent: HelloAck {
  ok: bool,
  error: Option<String>,
  server_version
}
...
- `client_id + psk` 是 MVP 的 agent 身份凭证；`client_id` 是公开标识，`psk` 是原始共享密钥，只参与 Noise 握手，不在网络中明文发送。
...
2. 实现 controller/agent TCP bridge Noise + PSK 握手和 `client_id` 校验。
